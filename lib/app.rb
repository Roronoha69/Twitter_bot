...
# Selectionne 5 journalistes aléatoire dans ma liste
 tweet_journalists = 5.times.map{ journalists[rand(0...journalists.length)] }.uniq

# Tweet les 5 journalistes selectionné
 tweet_journalists.each { |x| rest_connection.update("Nice article #{x} ! #Hello_world @the_hacking_pro")}

# like les 20 derniers tweets #bonjour_monde
 rest_connection.search("#bonjour_monde", result_type: "recent").take(20).collect do |tweet|
   rest_connection.fav(tweet)
 end

# Follow les 20 derniers journalistes ayant utilisé #bonjour_monde
rest_connection.search("#bonjour_monde", result_type: "recent").take(20).collect do |tweet|
 rest_connection.follow tweet.user
end

# Like et follow en temps réel les utilisateurs utilisant #bonjour_monde 
streaming_connection.filter(track: "#bonjour_monde") do |object|
 rest_connection.fav object
